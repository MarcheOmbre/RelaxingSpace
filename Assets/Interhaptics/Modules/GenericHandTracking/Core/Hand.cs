...
namespace Interhaptics.HandTracking
{
    ...
    public sealed class Hand
    {
        ...
        private static readonly UnityEngine.Vector2[] _fingerNormsDefault = {
            //THUMB
        ...
        public UnityEngine.Vector2?[] FingerNorms { get;  set; } = {
            null,
            null,
            null,
            null,
        ...
        public void SetHandSkeleton(UnityEngine.GameObject hand, Tools.HandMask.MaskPart handMask = Tools.HandMask.MaskPart.All, bool setPalm = false)
        {
            if (hand is null)
                return;
            ...
            if (handMask.HasFlag(Tools.HandMask.MaskPart.Thumb))
            {
                ...
                fingerIndex++)
            {
                ...
                if (!handMask.HasFlag((Tools.HandMask.MaskPart)(1 << (fingerIndex))))
                    continue;
        ...
        */
        ...
        public float GetFingerAngleState(Types.EFingers finger, bool normalize = true)
        {
            ...
            if (normalize)
                angle = Normalize(GetFingerNorm(finger).x, GetFingerNorm(finger).y, angle);

            return angle;
        }

        private UnityEngine.Vector2 GetFingerNorm(Types.EFingers _finger)
        {
            if ((int)_finger < FingerNorms.Length && FingerNorms[(int)_finger].HasValue)
                return FingerNorms[(int)_finger].Value;
            else if ((int)_finger < _fingerNormsDefault.Length)
                return _fingerNormsDefault[(int)_finger];
        ...
        public void PinchStrengthFromSkeleton()
        {
            float dist = UnityEngine.Vector3.Distance(
                ...
                GetFingerTipPosition((int) Types.EHandBones.HandThumbMetacarpal, (int) Types.EHandBones.HandThumbTip,
                    4));
            PinchStrength =
                UnityEngine.Mathf.Clamp(1 - Normalize(GetFingerNorm((Types.EFingers)5).x, GetFingerNorm((Types.EFingers)5).y, dist), 0, 1);
        }
        #endregion
