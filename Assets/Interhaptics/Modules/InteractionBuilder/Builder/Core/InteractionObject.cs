...
namespace Interhaptics.Modules.Interaction_Builder.Core
{
    ...
    public class InteractionObject : MonoBehaviour
    {
        ...
        /// <summary>
        ///     Event called the object position and rotation are computed
        /// </summary>
        [Tooltip("Event raised when the object position and rotation are computed")]
        public readonly UnityEvent OnObjectComputed = new UnityEvent();
        #endregion
        ...
        public bool FinishInteraction(BodyPartInteractionStrategy bodyPart, bool callEvent = true)
        {
        ...
        /// <param name="bodyPart">The body part with which interact</param>
        /// <returns>True if it was possible to finish the interaction, false otherwise</returns>
        public bool FinishInteraction(BodyPart bodyPart, bool callEvent = true)
        {
            return FinishInteraction(IbTools.Convert(bodyPart), callEvent);
        }
        ...
        /// <summary>
        /// Play the hands haptics during the interaction.
        /// </summary>
        /// <param name="movement">The object movement</param>
        private void PlayHandsHaptics(Vector3 movement)
        {
            List<int> humanBodyBones = new List<int>();
            bool isTwoHand = InteractWith == BodyPartInteractionStrategy.TwoHands || InteractWith == BodyPartInteractionStrategy.TwoHandsWithHead;
            bool isMultipleFingers = isTwoHand  || InteractWith == BodyPartInteractionStrategy.LeftHand || InteractWith == BodyPartInteractionStrategy.RightHand;

            if (isMultipleFingers)
            {
                if (isTwoHand || InteractWith == BodyPartInteractionStrategy.LeftHand)
                {
                    humanBodyBones.Add((int)HumanBodyBones.LeftThumbDistal);
                    humanBodyBones.Add((int)HumanBodyBones.LeftIndexDistal);
                    humanBodyBones.Add((int)HumanBodyBones.LeftMiddleDistal);
                    humanBodyBones.Add((int)HumanBodyBones.LeftRingDistal);
                    humanBodyBones.Add((int)HumanBodyBones.LeftLittleDistal);
                }
               
                if (isTwoHand || InteractWith == BodyPartInteractionStrategy.RightHand)
                {
                    humanBodyBones.Add((int)HumanBodyBones.RightThumbDistal);
                    humanBodyBones.Add((int)HumanBodyBones.RightIndexDistal);
                    humanBodyBones.Add((int)HumanBodyBones.RightMiddleDistal);
                    humanBodyBones.Add((int)HumanBodyBones.RightRingDistal);
                    humanBodyBones.Add((int)HumanBodyBones.RightLittleDistal);
                }
            }
            else
                humanBodyBones.Add(InteractWith.ToHumanBodyBonesValue());

            foreach(int humanBodyBone in humanBodyBones)
            {
                if(isMultipleFingers || this.IsAFinger(humanBodyBone))
                {
                    HapticRenderer.Core.HARWrapper.ComputeHaptics(HapticMaterialId, humanBodyBone, 
                        movement, interactionPrimitive.renderTexture, false, interactionPrimitive.renderVibration);

                    HapticRenderer.Core.HARWrapper.ComputeHaptics(HapticMaterialId, humanBodyBone-1,
                        movement, false, interactionPrimitive.renderStiffness, false);
                }
                else
                    HapticRenderer.Core.HARWrapper.ComputeHaptics(HapticMaterialId, humanBodyBone, movement, 
                        interactionPrimitive.renderTexture, interactionPrimitive.renderStiffness, interactionPrimitive.renderVibration);

            }
        }

        /// <summary>
        /// Check if the specified HumanBodyBones is a finger.
        /// </summary>
        /// <param name="bodypartID">The specified HumanBodyBones</param>
        /// <returns>True if the HumanBodyBones is a finger</returns>
        private bool IsAFinger(int bodypartID)
        {
            return (bodypartID == (int)HumanBodyBones.LeftIndexDistal
                    || bodypartID == (int)HumanBodyBones.LeftLittleDistal
                    || bodypartID == (int)HumanBodyBones.LeftMiddleDistal
                    || bodypartID == (int)HumanBodyBones.LeftRingDistal
                    || bodypartID == (int)HumanBodyBones.LeftThumbDistal
                    || bodypartID == (int)HumanBodyBones.RightIndexDistal
                    || bodypartID == (int)HumanBodyBones.RightLittleDistal
                    || bodypartID == (int)HumanBodyBones.RightMiddleDistal
                    || bodypartID == (int)HumanBodyBones.RightRingDistal
                    || bodypartID == (int)HumanBodyBones.RightThumbDistal);
        }
        #endregion
